...
/***/ function(module, exports, __webpack_require__) {
	...
	var Bonus = __webpack_require__(7);
	...
	Game.prototype.paddleBounce = function (x, dx, y, dy, radius) {
	  for (var i = 0; i < this.paddles.length; i++) {
	    ...
	    if ( y + dy > Game.DIM_Y - paddle.height - radius && x + dx > paddle.left && x + dx < paddle.left + paddle.width) {
	      return true;
	    }
	  }
	  ...
	  for (var i = 0; i < this.paddles.length; i++) {
	    ...
	    if ( bonus.y > Game.DIM_Y - paddle.height - bonus.radius && bonus.x > paddle.left && bonus.x < paddle.left + paddle.width) {
	      switch (bonus.prize) {
	        ...
	        // debugger
	            this.addCannonball({released: true});
	            this.addCannonball({released: true});
	          break;
	      }
	      // debugger
...
/***/ function(module, exports) {
	...
	Cannonball.prototype.move = function (timeChange) {
	  if (!this.released) {
	    ...
	    this.dx = this.dx || timeChange / 5;
	    this.dy = this.dy || -timeChange / 5;
